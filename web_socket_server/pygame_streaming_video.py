#!/usr/bin/env python3
"""
High-performance async MJPEG server with per-client frame caching,
YOLO object detection, and moving-average FPS.

Features:
- GPU/CPU canvas composition
- YOLOv8 inference (Ultralytics)
- nvImageCodec or CPU JPEG encoding
- Async non-blocking WebSocket commands
- Per-client FPS tracking and overlay
"""
import os
import asyncio
import logging
...
try:
    ...
    YOLO = None
    torch = None
    logging.warning("Ultralytics not installed. Run `pip install ultralytics torch` to enable YOLO.")
...
class Config:
    ...
    MAX_EXPECTED_CLIENTS = int(os.getenv("MAX_EXPECTED_CLIENTS", "4"))
...
class AppContext:
    ...
    def _load_yolo_model(self):
        if YOLO is None:
            logging.warning("YOLO not loaded — install ultralytics to enable.")
            return
        try:
            self.device = 'cuda' if self.have_cuda and torch.cuda.is_available() else 'cpu'
            self.model = YOLO("yolov8n.pt")  # lightweight model
            self.model.to(self.device)
            logging.info(f"YOLO model loaded on {self.device}")
        except Exception as e:
            logging.warning(f"YOLO model failed to load: {e}")
    ...
    async def handle_binary(self, msg, client_ip, request, ws):
        ...
        if self.ctx.model is not None:
            try:
                ...
                imgsz = (cfg.FRAME_WIDTH, int(np.ceil(cfg.FRAME_HEIGHT / 32) * 32))
                results = self.ctx.model.predict(cpu_frame, verbose=False, device=self.ctx.device, imgsz=imgsz)[0]
                for box in results.boxes:
                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                    conf = float(box.conf[0])
                    cls = int(box.cls[0])
                    label = f"{self.ctx.model.names[cls]} {conf:.2f}"
                    cv2.rectangle(cpu_frame, (x1, y1), (x2, y2), (0, 255, 0), 1)
                    cv2.putText(cpu_frame, label, (x1, y1 - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
                frame = cpu_frame
            except Exception as e:
                logging.warning(f"YOLO inference failed: {e}")
        ...
        ts = fq['timestamps']
        fq['fps'] = (len(ts)-1)/(ts[-1]-ts[0]) if len(ts) >= 2 and (ts[-1]-ts[0]) > 0 else 0.0
        fps_val = fq.get('fps', 0.0)
        hud_label = f"FPS:{fps_val:.1f}"
        cv2.putText(frame, hud_label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 0), 1)
